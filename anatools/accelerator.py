...
from cytoolz import curry, reduce, memoize, compose
...
OBJ = [T, T, T]
PINFO = Mapping
Accelerator = Callable[XYT, PINFO]
...
def compose_accelerators(accelerators: Iterable[Accelerator]) -> Callable[OBJ, PINFO]:
    wrapped: Sequence[Callable[[T, T], Callable[[PINFO], PINFO]]] = tuple(wrap_accelerator(acc) for acc in accelerators)

    @curry
    def accelerator(mass: T, charge: T, pz: T) -> PINFO:
...
@curry
@jit
def momentum_z(mass, charge, t, accelerator: Accelerator = None):
...
@curry
@jit
def momentum(x, y, t, accelerator=None, magnetic_filed=0, mass=1, charge=0):
